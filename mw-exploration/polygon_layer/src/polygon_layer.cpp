...
void PolygonLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  for (int j=min_j; j < max_j; j++)
  {
      ...
      {
          int min_index = getIndex(min_i, j);
          int max_index = getIndex(max_i, j);
          if (master[min_index] != LETHAL_OBSTACLE && (costmap_[min_index] == LETHAL_OBSTACLE || costmap_[min_index] > master[min_index]))
              master_grid.setCost(min_i, j, costmap_[min_index]);
          if (master[max_index] != LETHAL_OBSTACLE && (costmap_[max_index] == LETHAL_OBSTACLE || costmap_[max_index] > master[max_index]))
              master_grid.setCost(max_i, j, costmap_[max_index]);
      }
  }
}
