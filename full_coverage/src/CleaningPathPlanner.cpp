...
CleaningPathPlanning::CleaningPathPlanning(costmap_2d::Costmap2DROS *costmap2d_ros)
{
    ...
    SIZE_OF_CELL = 3;
    if (private_nh.searchParam("size_of_cell", sizeOfCellString))   //搜索参数
    ...
    for (i = 0; i < neuralizedMat.rows; i++)
    {
        for (j = 0; j < neuralizedMat.cols; j++)
        {
            if (cellMat.at<uchar>(i, j) == costmap_2d::LETHAL_OBSTACLE)
                ...
                neuralizedMat.at<float>(i, j) = 50.0 / j; //这里的1.0/j的用意是什么？这里有没有考虑到j=0时刻的问题呢？
        }
    }
    return;
}
...
void CleaningPathPlanning::mainPlanningLoop()
{
    ...
    const float c_0 = 5;              //default：0.001
    const float a_0 = 1;
    const float b_0 = 1;
        ...
        */
        ...
        for (int id = 0; id < 8; id++)
        {
            ...
            case 0:
                ...
                v = a_0 * neuralizedMat_.at<float>(currentPoint.row, currentPoint.col + 1) - b_0 * 1 + c_0 * e;
            ...
            case 1:
                ...
                v = a_0 * neuralizedMat_.at<float>(currentPoint.row + 1, currentPoint.col + 1) - b_0 * sqrt(2) + c_0 * e - 1000;
            ...
            case 2:
                ...
                v = a_0 * neuralizedMat_.at<float>(currentPoint.row - 1, currentPoint.col) - b_0 * 1  + c_0 * e;
