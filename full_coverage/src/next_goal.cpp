...
{ //当前位置回调函数,用来计算当前机器人位置与前面目标点的距离,判断是否要发新的目标点
...
int main(int argc, char *argv[])
{
  ...
  // ros::Subscriber sub1 = next_goal.subscribe("/odom", 10000, pose_callback);
  ros::Subscriber sub1 = next_goal.subscribe("/amcl_pose", 10000, amcl_pose_callback);
  ros::Subscriber sub2 = next_goal.subscribe("/path_planning_node/cleaning_plan_nodehandle/cleaning_path", 10000, path_callback);

  ros::Publisher pub1 = next_goal.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10000);
  ...
  while (ros::ok())
  {
    ...
    if (!planned_path.Path.empty())
    {
      ...
      if (goal_reached == false)
      {
        ...
        goal_msgs.pose.orientation.w = 1;
